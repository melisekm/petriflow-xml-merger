...
class PetriNet:
    def __init__(self, xml, increment_x=0, increment_y=0):
    ...
    def parse(self, rename=False, max_ids=None):
        if rename and not max_ids:
            raise ValueError("max_ids must be specified if renaming")
    ...
    def parse_keys(self, model_dict, rename, max_ids):
        for key, value in list(model_dict.items()):
            ...
            if key in ('x', 'y'):
                ...
                # split to string and int: p125 => ['p',125]
                match = SPLIT_PATTERN.match(value)
                name = match.group(1)
                idx = int(match.group(2))
                if rename:
                    # change name of current element to max + own value
